...
from project.task.mnist_small_samples.dataset import get_dataloader_generators
...
def dispatch_data(cfg: DictConfig, **kwargs: Any) -> DataStructure | None:
    ...
    iid: bool = dataset_config.get("iid")
    seed: int = dataset_config.get("seed")
    balance: bool = dataset_config.get("balance")
    num_samples: int = dataset_config.get("num_samples")
    ...
    if client_model_and_data is not None and dataset_dir is not None:
        ...
        if client_model_and_data.upper() == "MNIST_SMALL_CNN":
            return (
                get_net,
                client_dataloader_gen,
                fed_dataloader_gen,
        ...
        elif client_model_and_data.upper() == "MNIST_SMALL_LR":
            return (
